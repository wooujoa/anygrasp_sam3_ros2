#!/usr/bin/env python3
# AnyGrasp node for master_2 (LEFT ARM).
# - waits for /anygrasp_l_start true
# - consumes SAM3 output topics
# - runs AnyGrasp once per start signal
# - publishes the same intermediate topics used by CALI_D405:
#     /anygrasp_l/best_pose_raw
#     /anygrasp_l/best_contact_point
#     /anygrasp_l/best_width
#     /anygrasp_l/best_score
#     /anygrasp_l/grasps
#     visualization markers
# - publishes optional /anygrasp_l_finish
...
class AnyGraspMaster2Node(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().info('ANYGRASP MASTER2 Node Ready (LEFT ARM)')
    ...
    def start_callback(self, msg: Bool) -> None:
        if msg.data:
            ...
            self.active = True
            self.already_processed = False
            self.get_logger().info(f'[START] {self.start_topic} true. Cleared old inputs; waiting fresh SAM3 clouds.')
        else:
            self.active = False
            self.already_processed = False
            self.reset_runtime_inputs()
            self.get_logger().info(f'[STOP] {self.start_topic} false. paused and input cache cleared.')
